...
def extract_cash_flow(symbols: list[str] | None = None, run_id: str | None = None):
    logger = setup_logger()
    ...
    logger.info(f"Starting extraction for cash_flow for {len(symbols)} symbols: {symbols}")
    files_generated = []
    ...
    watermark = WatermarkManager(gcp_loader=gcp_loader, endpoint="cash_flow")
    ...
    for symbol in symbols:
        ...
        try:
            ...
            logger.info(f"Validating data for cash_flow for {symbol}...")
            ...
            time_seconds = time.perf_counter() - start_time
            logger.error(f"Unexpected error while extracting cash_flow for {symbol}: {e}")
            log_extraction(
                logger=logger,
                status="ERROR",
                stage_location_bucket=BUCKET_BRONZE,
