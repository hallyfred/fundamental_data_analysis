...
from src.extract.contract import CashFlowSchema
...
def extract_cash_flow():
    logger = setup_logger()
    print( "Extracting cash_flow data for symbols...")
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            print(f"Validating cash_flow data for {symbol}...")
            extraction_date = today.isoformat()

            validated_data = CashFlowSchema.model_validate(files)
            ...
            destination_blob_name = f"financial/cash_flow/year={today.year}/month={today.month:02d}/day={today.day:02d}/{file_name}"
            ...
            time_seconds = time.perf_counter() - start_time
            print(f"Error occurred while extracting cash_flow data for {symbol}: {e}")
