...
from src.extract.contract import IncomeStatementSchema
...
def extract_income_statement():
    logger = setup_logger()
    print( "Extracting income_statement data for symbols...")
    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            print(f"Validating income_statement data for {symbol}...")
            extraction_date = today.isoformat()

            validated_data = IncomeStatementSchema.model_validate(files)
            ...
            time_seconds = time.perf_counter() - start_time
            print(f"Error occurred while extracting income_statement data for {symbol}: {e}")
    ...
    log_destination = f"financial/metadata/income_statement/year={today.year}/month={today.month:02d}/day={today.day:02d}/income_statement_extraction.log"
