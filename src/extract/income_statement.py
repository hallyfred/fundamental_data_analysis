...
def extract_income_statement(symbols: list[str] | None = None, run_id: str | None = None):
    ...
    logger.info(f"Starting extraction for income_statement for {len(symbols)} symbols: {symbols}")
    files_generated = []
    ...
    for symbol in symbols:
        ...
        try:
            ...
            logger.info(f"Validating data for income_statement for {symbol}...")
            ...
            time_seconds = time.perf_counter() - start_time
            logger.error(f"Unexpected error while extracting income_statement for {symbol}: {e}")
            log_extraction(
                logger=logger,
                status="ERROR",
                stage_location_bucket=BUCKET_BRONZE,
