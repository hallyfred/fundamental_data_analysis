...
- **Produção local preparada**: segredos retirados do Compose e rotacionados, Airflow restrito a `127.0.0.1:8081`, PostgreSQL sem porta publicada, target dbt `prod` aprovado, três serviços saudáveis, backup validado, build limpo concluído e suíte Linux com 67 testes aprovados. Permanece somente a evidência real de sete dias e idempotência após a renovação da cota (T059/T042), antes de liberar o merge final.
...
- [x] T056 Tornar a imagem Docker reproduzível com dependências compatíveis e travadas, executar um build limpo do zero e validar importação da DAG, Ruff, pytest e dbt no container resultante. Resultado: base Airflow 2.9.3 fixada por digest, 392 dependências travadas, providers Google compatibilizados com protobuf 5, imagem compartilhada pelos serviços e build `--no-cache` concluído em cerca de dois minutos; `pip check`, imports Airflow/Cosmos/httpx, DAG sem erros, Ruff, formatação, 67 testes pytest e `dbt parse --target prod` aprovados. O CI cria um `.env` vazio e ignorado antes de validar o Compose, enquanto os valores dummy continuam definidos no ambiente do job.
...
- **Phase 16 (Produção local)**: T054–T058 e T060 concluídas; T059 aguarda nova janela de cota e sete dias de evidências reais antes do release na `main`
